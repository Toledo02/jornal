...
python -m pytest -q                   # 31 testes, sem rede
```
...
**Observabilidade não é opcional aqui.** Toda seção `error` ou `partial` gera alerta no Telegram
...
Ao adicionar uma fonte, garanta que a falha dela apareça no `status`, não só no log.
...
para os alvos de scraping de HTML — InfoMoney, GE Globo, Mercado Livre, Buscapé e GitHub Trending.
...
**Scrapers de HTML são frágeis por natureza.** Futebol (GE), promoções (Mercado Livre/Buscapé) e
...
**Estado local em `logs/`:** `last_journal.txt` alimenta a regra anti-repetição e só é gravado após
envio bem-sucedido; `promotions_history.json` (raiz) guarda o preço anterior de cada produto.
